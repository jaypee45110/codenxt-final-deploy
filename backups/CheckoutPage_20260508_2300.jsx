...
const compressImageDataUrl = (dataUrl, maxWidth = 900, maxHeight = 900, quality = 0.72) =>
  new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      if (output.length > 110000) {
        output = canvas.toDataURL('image/jpeg', 0.45);
      }
  ...
  export default function Checkout() {
    useEffect(() => {
      document.title = '🧾 Checkout — codeTone';
      }, []);
...
const copy = {
  en: {
    ...
    subtitle:
      'Complete the order setup, choose arrangement type, review terms, and proceed to your control center.',
  },
  no: {
    eyebrow: 'Checkout',
...
const handleContinue = async () => {
  ...
  const payload = {
    ...
    audienceSize: formData.audienceSize.trim(),
  ...
  payload.artistLogo = await compressImageDataUrl(payload.artistLogo);
  try {
...
body: JSON.stringify({
  ...
  venue: payload.venue,
city: payload.city,
  badgeConfig: JSON.parse(localStorage.getItem("codenxt_badge_config") || '{"template":"americana"}'),
startAt: `${formData.eventDate || '2026-07-02'}T20:00:00.000Z`,
...
if (!eventRes.ok) {
  console.error('CREATE EVENT RAW RESPONSE:', rawEventResponse);
  throw new Error(`Create event failed: ${eventRes.status}`);
}

const eventData = JSON.parse(rawEventResponse);
    console.log('CREATE EVENT RESPONSE:', eventData);

    const finalEventCode = eventData?.event?.code || eventData?.code || eventCode;

    payload.eventCode = finalEventCode;
    payload.shortLink = `codetone.codenxt.global/join/${finalEventCode}`;

    try {
      localStorage.setItem('codenxt_event', JSON.stringify(payload));
    } catch (error) {
      console.error('Could not update saved event locally:', error);
    }

    localStorage.setItem('codenxt_checkout_locked', '1');
    localStorage.setItem('codenxt_active_event_code', finalEventCode);

navigate(`/dashboard?event=${finalEventCode}&t=${Date.now()}`, {
  state: { ...payload, eventCode: finalEventCode },
  replace: true,
});
  } catch (error) {
    setSubmitting(false);
        ...
        <div style={styles.contentGrid}>
          <div style={styles.leftColumn}>
            <div style={styles.card}>
              <div style={styles.cardTitle}>1.Customer information</div>
            ...
            <div style={styles.card}>
              <div style={styles.cardTitle}>2.Event details</div>
              ...
              <div style={styles.twoCol}>
                ...
                <div style={{ ...styles.fieldBlock, gridColumn: '1 / -1' }}>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                  />
                  {triedSubmit && (!formData.audienceSize.trim() || Number(formData.audienceSize) <= 0) && (
                  <div style={styles.errorText}>Expected audience is required.</div>
                  )}
                </div>
          ...
          <div style={styles.rightColumn}>
            <div style={styles.card}>
              <div style={styles.cardTitle}>3.Arrangement type</div>
            ...
            <div style={styles.card}>
              <div style={styles.cardTitle}>4.Simulated payment</div>
              <p style={styles.cardText}>
                This is a visual placeholder for the DEMO version.Not neccesary to fill in
              </p>
              ...
              <div style={styles.fakePaymentBox}>
                ...
                <div style={styles.fakePaymentText}>
                  Payment is not processed in this DEMO version. This step only simulates the
            ...
            <div style={styles.card}>
              <div style={styles.cardTitle}>5.Terms and Conditions</div>
              <p style={styles.cardText}>
            ...
            <div style={styles.card}>
              <div style={styles.cardTitle}>6.Next step</div>
              <p style={styles.cardText}>
                You’re ready to go live. Best experienced on a laptop.              </p>
