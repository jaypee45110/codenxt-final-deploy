...
try {
  ...
  const backendCode = eventData?.event?.code || eventData?.code;

  if (backendCode) {
    eventCode = backendCode;
    payload = {
      ...payload,
      eventCode: backendCode,
      shortLink: `codetone.codenxt.global/join/${backendCode}`,
    };

    try {
      localStorage.setItem('codenxt_event', JSON.stringify(payload));
    } catch (error) {
      console.error('Could not update local event with backend code:', error);
    }
  }
} catch (error) {
  console.error('Could not create backend event:', error);
}
