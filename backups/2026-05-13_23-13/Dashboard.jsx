...
const [uploadedRewards, setUploadedRewards] = useState({
  ...
  const [goldPercent, setGoldPercent] = useState(0.25);
...
shortLink: '',
  ...
  const audienceSizeNumber = Number(eventData?.audienceSize || 0);
  ...
  const estimatePeople = (percent) => {
    if (!audienceSizeNumber || audienceSizeNumber <= 0) return null;
    return Math.round((audienceSizeNumber * Number(percent || 0)) / 100);
  };
...
useEffect(() => {
  ...
  // Soft circular base for lure field, without hard white outer ring
  ctx.fillStyle = 'rgba(255, 255, 255, 0.92)';
  ctx.beginPath();
  ctx.arc(center, center + 4, outerRadius - moduleSize * 2.8, 0, Math.PI * 2);
  ctx.fill();

  // Fake circular lure field
...
const handleSendInnerCircleSms = async () => {
  if (!eventData?.eventCode) {
    setSmsStatus('Missing event code.');
    return;
  }
  ...
  try {
    const res = await fetch(`${API_BASE}/send-innercircle-sms`, {
      ...
      body: JSON.stringify({ eventCode: eventData.eventCode }),
    });

    const data = await res.json();

    if (!res.ok || !data.ok) {
      setSmsStatus(`SMS failed: ${data?.error || data?.message || 'Unknown error'}`);
      return;
    }

    setSmsStatus(`Sent ${data.sentCount || 0} SMS${data.failedCount ? `, ${data.failedCount} failed` : ''}.`);
  } catch (err) {
    console.error('Send SMS failed:', err);
    setSmsStatus('SMS failed. See console.');
  } finally {
    setSmsSending(false);
  }
};
...
{/* Metrics moved below summary header */}
        </div>
      </div>
...
countdownNumber.innerHTML = 'THANK YOU<br/>FOR TONIGHT';
setTimeout(() => {
...
>
  ...
  <div className="actions-grid">
    ...
    <button
      ...
      onClick={handleSendInnerCircleSms}
      disabled={smsSending}
    >
...
alignItems: 'center',
  ...
  >
    <span style={{ color: '#ffd94d' }}>GOLD: first {formatAllocationPercent(goldPercent)}%{estimatePeople(goldPercent) !== null ? ` · approx. ${estimatePeople(goldPercent)} people` : ''}</span>
    <span style={{ color: '#d7dde5' }}>SILVER: next {formatAllocationPercent(silverPercent)}%{estimatePeople(silverPercent) !== null ? ` · approx. ${estimatePeople(silverPercent)} people` : ''}</span>
...
>
  ...
  >
    <button
      ...
      onClick={() => {
        if (activeAllocationTier === 'gold') {
          setGoldPercent(Math.max(0, Math.round((goldPercent - 0.25) * 100) / 100));
        }

        if (activeAllocationTier === 'silver') {
          setSilverPercent(Math.max(0, Math.round((silverPercent - 0.25) * 100) / 100));
        }
      }}
      style={{
        width: 34,
    ...
    <button
      ...
      onClick={() => {
        if (activeAllocationTier === 'gold') {
          setGoldPercent(Math.min(100, Math.round((goldPercent + 0.25) * 100) / 100));
        }

        if (activeAllocationTier === 'silver') {
          setSilverPercent(Math.min(100, Math.round((silverPercent + 0.25) * 100) / 100));
        }
      }}
      style={{
        width: 34,
