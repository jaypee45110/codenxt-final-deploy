...
export default function Dashboard() {
  ...
  const [selectedRewardType, setSelectedRewardType] = useState('image');
  const [selectedAccessMode, setSelectedAccessMode] = useState('moment');
...
try {
  ...
  console.error('Join count error:', e);
}
    return [
      {
        label: t.tickets,
...
const rewardData = (() => {
  ...
  const handleUploadReward = () => {
    let rewardUrl = setlistImg;

if (selectedFile) {
  rewardUrl = URL.createObjectURL(selectedFile);
  ...
  if (!qrMarkup && !qrImageSrc) {
    ...
    printWindow.document.write(`
      ...
      <html>
        ...
        <body>
          <div class="poster">
            <div class="top">SCAN ME</div>
            <div class="sub">TO CONNECT</div>

<div class="qrWrap">
  <div class="qrFrame">
    ${qrMarkup || `<img src="${qrImageSrc}" alt="Round QR" />`}
  </div>
</div>
            <div class="bottom">${eventTitle}</div>
            ${venueName ? '<div class="venue">' + venueName + '</div>' : ''}
...
let innerCirclePhones = [];

try {
  const allInnerCircle = JSON.parse(localStorage.getItem('innerCircle') || '[]');
  const eventInnerCircle = allInnerCircle.filter(
    (entry) => entry.eventCode === eventData.eventCode
  );

  innerCircleCount = eventInnerCircle.length;
  innerCirclePhones = eventInnerCircle.map((entry) => entry.phone);
} catch (error) {
  console.error('Could not parse innerCircle data:', error);
}
    if (reportRaw) {
      try {
        const reportData = JSON.parse(reportRaw);
        joinCount = Array.isArray(reportData?.joins) ? reportData.joins.length : 0;
      } catch (error) {
        console.error('Could not parse report data:', error);
      }
    }

    const reportWindow = window.open('', '_blank', 'width=1100,height=1400');    
    if (!reportWindow) return;

    reportWindow.document.write(`
      <html>
        <head>
          <title>codeTone Live Interaction Report</title>
          <style>
            body {
              margin: 0;
              background: #f3f5f7;
...
.brandline {
        ...
        <body>
          <div class="page">
            <div class="eyebrow">codeTone 20©26 · Live Event Technology</div>
<div class="brandline">codeTone by codeNXT</div>

            <h1 class="title">Live Interaction Report</h1>
            <div class="subtitle">${eventData.artistName || 'Artist / Event Name'} · ${eventData.venue || 'Venue'}${eventData.city ? ', ' + eventData.city : ''}</div>
            <div class="hero">
              <h2 class="section-title">About This Report</h2>
              <p class="text">
                This report summarizes the current codeTone activation state for the selected event. It captures the
                live access setup, reward readiness, screen state and current InnerCircle join activity in one clean operational view.
              </p>
            </div>
            ...
            <div class="grid">
              <div class="card">
                ...
                <div class="value">${eventData.eventCode || ''}</div>
                <div class="small">Unique event identifier used in the live flow.</div>
              </div>

              <div class="card">
                <div class="label">Location</div>
                <div class="value">${eventData.venue || 'Venue'}</div>
                <div class="small">${eventData.city || 'City'}</div>
              </div>
            </div>

            <h2 class="section-title">Activation Metrics</h2>

            <div class="metrics">
              <div class="metric">
                <div class="metric-number">${rewardUploaded ? 'YES' : 'NO'}</div>
                <div class="metric-label">Reward Uploaded</div>
              </div>

              <div class="metric">
                <div class="metric-number">${rewardUnlocked ? 'LIVE' : 'WAITING'}</div>
                <div class="metric-label">Reward Status</div>
              </div>

              <div class="metric">
                <div class="metric-number">${screenLive ? 'ON' : 'OFF'}</div>
                <div class="metric-label">Screen Status</div>
              </div>

              <div class="metric">
                <div class="metric-number">${joinCount}</div>
                <div class="metric-label">InnerCircle Joins</div>
              </div>
              <div class="metric">
  <div class="metric-number">${innerCircleCount}</div>
  <div class="metric-label">Confirmed Mobile Numbers</div>
</div>
            </div>
<h2 class="section-title">InnerCircle Mobile List</h2>
<p class="text">
  The following mobile numbers were confirmed for this event:
</p>

<div class="card">
  <div class="label">Confirmed Numbers</div>
  <div class="small" style="margin-top: 10px; white-space: pre-line;">
    ${innerCirclePhones.length ? innerCirclePhones.join('<br />') : 'No mobile numbers confirmed yet.'}
  </div>
</div>
            <h2 class="section-title">Operational Summary</h2>
            <p class="text">
              The event is currently set up for a codeTone live interaction flow in which the audience scans a QR code,
              enters the join page, accepts consent, and receives access to a timed digital reward.
            </p>
            <p class="text">
              At the moment this report was generated, the reward was <strong>${rewardUploaded ? 'stored in the system' : 'not yet stored'}</strong>,
              the live reward state was <strong>${rewardUnlocked ? 'open' : 'not yet opened'}</strong>,
              and the venue screen status was <strong>${screenLive ? 'active' : 'inactive'}</strong>.
            </p>

            <div class="footer">
              Generated from codeTone dashboard · Event code ${eventData.eventCode || ''} · ${new Date().toLocaleString()}
            </div>
          </div>
        </body>
      </html>
...
{(rewardData || selectedRewardType || selectedAccessMode) && (
